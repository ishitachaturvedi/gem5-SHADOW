...
int main(int argc, char *argv[]) {
    ...
    int numNonZeroA = ROWS_A * COLS_A / val_non_zero; // Roughly 10% non-zero elements in A
    int numNonZeroB = ROWS_B * COLS_B / val_non_zero; // Roughly 10% non-zero elements in B
    ...
    // Workload distribution: The first thread handles half the work, others share the rest
    int totalRows = A.rows;
    int firstThreadRows = totalRows / 2;  // First thread handles half the rows
    int remainingRows = totalRows - firstThreadRows;
    int otherThreadRows = remainingRows / (NUM_THREADS - 1);  // Distribute remaining rows among other threads

    // Create and start threads
    threadArgs[0].A = &A;
    threadArgs[0].B = &B;
    threadArgs[0].C = &C;
    threadArgs[0].startRow = 0;
    threadArgs[0].endRow = firstThreadRows;
    pthread_create(&threads[0], NULL, multiplySparseMatrices, (void *)&threadArgs[0]);

    for (int i = 1; i < NUM_THREADS; i++) {
        threadArgs[i].A = &A;
        threadArgs[i].B = &B;
        threadArgs[i].C = &C;
        threadArgs[i].startRow = firstThreadRows + (i - 1) * otherThreadRows;
        threadArgs[i].endRow = (i == NUM_THREADS - 1) ? totalRows : firstThreadRows + i * otherThreadRows;
